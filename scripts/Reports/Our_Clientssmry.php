...
<?php include_once "Our_Clientssmryinfo.php" ?>
<?php
...
$Our_Clients_summary = NULL; // Initialize page object first

class crOur_Clients_summary extends crOur_Clients {
	...
	// Page object name
	var $PageObjName = 'Our_Clients_summary';
	...
	function __construct() {
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'Our Clients', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fOur_Clientssummary";
...
//		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewrExportLink ewEmail\" title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_Our_Clients\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_Our_Clients',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fOur_Clientssummary\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fOur_Clientssummary\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fOur_Clientssummary\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function Page_Main() {
		...
		$this->relationship_owner->SetVisibility();
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['relationship_owner'] = ewr_Conv($rs->fields('relationship_owner'), 200);
				$this->FirstRowData['ref'] = ewr_Conv($rs->fields('ref'), 200);
		...
		if (!$rs->EOF) {
			...
			$this->relationship_owner->setDbValue($rs->fields('relationship_owner'));
			$this->ref->setDbValue($rs->fields('ref'));
			$this->city->setDbValue($rs->fields('city'));
			$this->Val[1] = $this->company_name->CurrentValue;
			$this->Val[2] = $this->relationship_owner->CurrentValue;
			$this->Val[3] = $this->city->CurrentValue;
		} else {
			$this->company_name->setDbValue("");
			$this->relationship_owner->setDbValue("");
			$this->ref->setDbValue("");
	...
	function SetupPopup() {
		...
		if (is_array(@$_SESSION["sel_Our_Clients_city"])) {
			$this->LoadSelectionFromSession('city');
		} elseif (@$_SESSION["sel_Our_Clients_city"] == EWR_INIT_VALUE) { // Select all
			$this->city->SelectionList = "";
		}
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			// relationship_owner
			$this->relationship_owner->HrefValue = "";
			...
			// relationship_owner
			$this->relationship_owner->ViewValue = $this->relationship_owner->CurrentValue;
			$this->relationship_owner->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// relationship_owner
			$CurrentValue = $this->relationship_owner->CurrentValue;
			$ViewValue = &$this->relationship_owner->ViewValue;
			$ViewAttrs = &$this->relationship_owner->ViewAttrs;
			$CellAttrs = &$this->relationship_owner->CellAttrs;
			$HrefValue = &$this->relationship_owner->HrefValue;
			$LinkAttrs = &$this->relationship_owner->LinkAttrs;
			$this->Cell_Rendered($this->relationship_owner, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
	...
	function SetupFieldCount() {
		...
		if ($this->relationship_owner->Visible) $this->DtlColumnCount += 1;
	...
	function GetExtendedFilter() {
		...
		if ($bPostBack) {
			...
			if ($this->PopupName == 'Our_Clients_city' && $this->PopupValue <> "") {
			...
			} elseif ($this->ClearExtFilter == 'Our_Clients_city') {
		...
		// Reset search command
			...
			if ($this->GetDropDownValue($this->city)) {
				$bSetupFilter = TRUE;
			} elseif ($this->city->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_Our_Clients_city'])) {
				$bSetupFilter = TRUE;
			}
		...
		if ($bSetupFilter) {
			...
			$_SESSION['sel_Our_Clients_city'] = ($this->city->SelectionList == "") ? EWR_INIT_VALUE : $this->city->SelectionList;
		}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_Our_Clients_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_Our_Clients_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_Our_Clients_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_Our_Clients_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_Our_Clients_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_Our_Clients_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_Our_Clients_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_Our_Clients_' . $parm] = $sv;
		$_SESSION['so_Our_Clients_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_Our_Clients_' . $parm] = $sv1;
		$_SESSION['so_Our_Clients_' . $parm] = $so1;
		$_SESSION['sc_Our_Clients_' . $parm] = $sc;
		$_SESSION['sv2_Our_Clients_' . $parm] = $sv2;
		$_SESSION['so2_Our_Clients_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_Our_Clients_$parm"] = "";
		$_SESSION["rf_Our_Clients_$parm"] = "";
		$_SESSION["rt_Our_Clients_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_Our_Clients_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_Our_Clients_$parm"];
		$fld->RangeTo = @$_SESSION["rt_Our_Clients_$parm"];
	}
	...
	function SetupFilterList($filter) {
		...
		if (array_key_exists("sel_city", $filter)) {
			...
			$_SESSION["sel_Our_Clients_city"] = $sWrk;
	...
	function GetSort($options = array()) {
		...
		if ($bResetSort) {
			...
			$this->relationship_owner->setSort("");
			$this->city->setSort("");
...
// Create page object
if (!isset($Our_Clients_summary)) $Our_Clients_summary = new crOur_Clients_summary();
if (isset($Page)) $OldPage = $Page;
$Page = &$Our_Clients_summary;
...
// Create page object
var Our_Clients_summary = new ewr_Page("Our_Clients_summary");

// Page properties
Our_Clients_summary.PageID = "summary"; // Page ID
var EWR_PAGE_ID = Our_Clients_summary.PageID;
</script>
<?php } ?>
...
// Form object
var CurrentForm = fOur_Clientssummary = new ewr_Form("fOur_Clientssummary");

// Validate method
fOur_Clientssummary.Validate = function() {
...
fOur_Clientssummary.Form_CustomValidate = 
...
fOur_Clientssummary.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fOur_Clientssummary.ValidateRequired = false; // No JavaScript validation
<?php } ?>

// Use Ajax
fOur_Clientssummary.Lists["sv_city"] = {"LinkField":"sv_city","Ajax":true,"DisplayFields":["sv_city","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown && !$Page->ShowReportInDashboard) { ?>
<script type="text/javascript">
...
<?php if ($Page->Export == "" && !$Page->DrillDown && !$Page->ShowReportInDashboard) { ?>
<!-- Search form (begin) -->
<form name="fOur_Clientssummary" id="fOur_Clientssummary" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fOur_Clientssummary_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<?php $Page->city->EditAttrs["onchange"] = "ewrForms(this).Submit(); " . @$Page->city->EditAttrs["onchange"]; ?>
<?php ewr_PrependClass($Page->city->EditAttrs["class"], "form-control"); ?>
<select data-table="Our_Clients" data-field="x_city" data-value-separator="<?php echo ewr_HtmlEncode(is_array($Page->city->DisplayValueSeparator) ? json_encode($Page->city->DisplayValueSeparator) : $Page->city->DisplayValueSeparator) ?>" id="sv_city" name="sv_city"<?php echo $Page->city->EditAttributes() ?>>
...
<script type="text/javascript">
fOur_Clientssummary.Lists["sv_city"].Options = <?php echo ewr_ArrayToJson($Page->city->LookupFilterOptions) ?>;
</script>
</span>
</div>
</div>
...
<script type="text/javascript">
fOur_Clientssummary.Init();
fOur_Clientssummary.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php } ?>
<!-- Report grid (begin) -->
<div id="gmp_Our_Clients" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="company_name"><div class="Our_Clients_company_name"><span class="ewTableHeaderCaption"><?php echo $Page->company_name->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="company_name">
<?php if ($Page->SortUrl($Page->company_name) == "") { ?>
		<div class="ewTableHeaderBtn Our_Clients_company_name">
			<span class="ewTableHeaderCaption"><?php echo $Page->company_name->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Our_Clients_company_name" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->company_name) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->company_name->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->relationship_owner->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="relationship_owner"><div class="Our_Clients_relationship_owner"><span class="ewTableHeaderCaption"><?php echo $Page->relationship_owner->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="relationship_owner">
<?php if ($Page->SortUrl($Page->relationship_owner) == "") { ?>
		<div class="ewTableHeaderBtn Our_Clients_relationship_owner">
			<span class="ewTableHeaderCaption"><?php echo $Page->relationship_owner->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Our_Clients_relationship_owner" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->relationship_owner) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->relationship_owner->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->relationship_owner->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->relationship_owner->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="city"><div class="Our_Clients_city"><span class="ewTableHeaderCaption"><?php echo $Page->city->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="city">
<?php if ($Page->SortUrl($Page->city) == "") { ?>
		<div class="ewTableHeaderBtn Our_Clients_city">
			<span class="ewTableHeaderCaption"><?php echo $Page->city->FldCaption() ?></span>
	<?php if (!$Page->ShowReportInDashboard) { ?>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, { name: 'Our_Clients_city', range: false, from: '<?php echo $Page->city->RangeFrom; ?>', to: '<?php echo $Page->city->RangeTo; ?>', url: 'Our_Clientssmry.php' });" id="x_city<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Our_Clients_city" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->city) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->city->FldCaption() ?></span>
...
<?php } ?>
<?php if ($Page->relationship_owner->Visible) { ?>
		<td data-field="relationship_owner"<?php echo $Page->relationship_owner->CellAttributes() ?>>
<span<?php echo $Page->relationship_owner->ViewAttributes() ?>><?php echo $Page->relationship_owner->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->city->Visible) { ?>
...
<?php include "Our_Clientssmrypager.php" ?>
