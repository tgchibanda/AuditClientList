...
<?php include_once "IRBA_With_Searchsmryinfo.php" ?>
<?php
...
$IRBA_With_Search_summary = NULL; // Initialize page object first

class crIRBA_With_Search_summary extends crIRBA_With_Search {
	...
	// Page object name
	var $PageObjName = 'IRBA_With_Search_summary';
	...
	function __construct() {
		...
		// Table object (IRBA_With_Search)
		if (!isset($GLOBALS["IRBA_With_Search"])) {
			$GLOBALS["IRBA_With_Search"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["IRBA_With_Search"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'IRBA With Search', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fIRBA_With_Searchsummary";
...
//		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewrExportLink ewEmail\" title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_IRBA_With_Search\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_IRBA_With_Search',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fIRBA_With_Searchsummary\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fIRBA_With_Searchsummary\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fIRBA_With_Searchsummary\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['Resigned'] = ewr_Conv($rs->fields('Resigned'), 200);
		} else { // Get next row
			$rs->MoveNext();
		}
		if (!$rs->EOF) {
			...
			$this->Resigned->setDbValue($rs->fields('Resigned'));
			...
			$this->Val[19] = $this->Resigned->CurrentValue;
		} else {
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			// Resigned
			$this->Resigned->ViewValue = $this->Resigned->CurrentValue;
			$this->Resigned->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// Resigned
			$CurrentValue = $this->Resigned->CurrentValue;
			$ViewValue = &$this->Resigned->ViewValue;
			$ViewAttrs = &$this->Resigned->ViewAttrs;
			$CellAttrs = &$this->Resigned->CellAttrs;
			$HrefValue = &$this->Resigned->HrefValue;
			$LinkAttrs = &$this->Resigned->LinkAttrs;
			$this->Cell_Rendered($this->Resigned, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
		}
	...
	function SetupFieldCount() {
		...
		if ($this->Resigned->Visible) $this->DtlColumnCount += 1;
	}
	...
	function GetExtendedFilter() {
		...
		// Reset search command
			...
			// Load default values
			$this->SetSessionDropDownValue($this->Resigned->DropDownValue, $this->Resigned->SearchOperator, 'Resigned'); // Field Resigned
			...
			// Field Resigned
			if ($this->GetDropDownValue($this->Resigned)) {
				$bSetupFilter = TRUE;
			} elseif ($this->Resigned->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_IRBA_With_Search_Resigned'])) {
				$bSetupFilter = TRUE;
			}
		...
		// Restore session
		if ($bRestoreSession) {
			$this->GetSessionDropDownValue($this->Resigned); // Field Resigned
		}
		...
		// Build SQL
		$this->BuildDropDownFilter($this->Resigned, $sFilter, $this->Resigned->SearchOperator, FALSE, TRUE); // Field Resigned
		...
		// Field Resigned
		ewr_LoadDropDownList($this->Resigned->DropDownList, $this->Resigned->DropDownValue);
		return $sFilter;
	}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_IRBA_With_Search_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_IRBA_With_Search_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_IRBA_With_Search_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_IRBA_With_Search_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_IRBA_With_Search_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_IRBA_With_Search_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_IRBA_With_Search_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_IRBA_With_Search_' . $parm] = $sv;
		$_SESSION['so_IRBA_With_Search_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_IRBA_With_Search_' . $parm] = $sv1;
		$_SESSION['so_IRBA_With_Search_' . $parm] = $so1;
		$_SESSION['sc_IRBA_With_Search_' . $parm] = $sc;
		$_SESSION['sv2_IRBA_With_Search_' . $parm] = $sv2;
		$_SESSION['so2_IRBA_With_Search_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_IRBA_With_Search_$parm"] = "";
		$_SESSION["rf_IRBA_With_Search_$parm"] = "";
		$_SESSION["rt_IRBA_With_Search_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_IRBA_With_Search_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_IRBA_With_Search_$parm"];
		$fld->RangeTo = @$_SESSION["rt_IRBA_With_Search_$parm"];
	}
	...
	function LoadDefaultFilters() {
		...
		*/
		...
		// Field Resigned
		$this->Resigned->DefaultDropDownValue = EWR_INIT_VALUE;
		if (!$this->SearchCommand) $this->Resigned->DropDownValue = $this->Resigned->DefaultDropDownValue;
		/**
	...
	function CheckFilter() {
		...
		// Check Resigned extended filter
		if ($this->NonTextFilterApplied($this->Resigned))
			return TRUE;
		return FALSE;
	}
	...
	function ShowFilterList($showDate = FALSE) {
		...
		$sExtWrk = "";
		$sWrk = "";
		$this->BuildDropDownFilter($this->Resigned, $sExtWrk, $this->Resigned->SearchOperator);
		$sFilter = "";
		if ($sExtWrk <> "")
		...
		if ($sFilter <> "")
			$sFilterList .= "<div><span class=\"ewFilterCaption\">" . $this->Resigned->FldCaption() . "</span>" . $sFilter . "</div>";
		$divstyle = "";
		$divdataclass = "";
	...
	function GetFilterList() {
		...
		// Field Resigned
		$sWrk = "";
		$sWrk = ($this->Resigned->DropDownValue <> EWR_INIT_VALUE) ? $this->Resigned->DropDownValue : "";
		if (is_array($sWrk))
			$sWrk = implode("||", $sWrk);
		if ($sWrk <> "")
			$sWrk = "\"sv_Resigned\":\"" . ewr_JsEncode2($sWrk) . "\"";
		if ($sWrk <> "") {
	...
	function SetupFilterList($filter) {
		...
		if (array_key_exists("sv_Resigned", $filter)) {
			$sWrk = $filter["sv_Resigned"];
			if (strpos($sWrk, "||") !== FALSE)
				$sWrk = explode("||", $sWrk);
			$this->SetSessionDropDownValue($sWrk, @$filter["so_Resigned"], "Resigned");
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionDropDownValue(EWR_INIT_VALUE, "", "Resigned");
		}
		return TRUE;
	}
...
// Create page object
if (!isset($IRBA_With_Search_summary)) $IRBA_With_Search_summary = new crIRBA_With_Search_summary();
if (isset($Page)) $OldPage = $Page;
$Page = &$IRBA_With_Search_summary;
...
// Create page object
var IRBA_With_Search_summary = new ewr_Page("IRBA_With_Search_summary");

// Page properties
IRBA_With_Search_summary.PageID = "summary"; // Page ID
var EWR_PAGE_ID = IRBA_With_Search_summary.PageID;
</script>
<?php } ?>
...
// Form object
var CurrentForm = fIRBA_With_Searchsummary = new ewr_Form("fIRBA_With_Searchsummary");

// Validate method
fIRBA_With_Searchsummary.Validate = function() {
...
fIRBA_With_Searchsummary.Form_CustomValidate = 
...
fIRBA_With_Searchsummary.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fIRBA_With_Searchsummary.ValidateRequired = false; // No JavaScript validation
<?php } ?>

// Use Ajax
fIRBA_With_Searchsummary.Lists["sv_Resigned"] = {"LinkField":"sv_Resigned","Ajax":true,"DisplayFields":["sv_Resigned","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown && !$Page->ShowReportInDashboard) { ?>
<script type="text/javascript">
...
<?php if ($Page->Export == "" && !$Page->DrillDown && !$Page->ShowReportInDashboard) { ?>
<!-- Search form (begin) -->
<form name="fIRBA_With_Searchsummary" id="fIRBA_With_Searchsummary" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fIRBA_With_Searchsummary_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<div id="r_1" class="ewRow">
<div id="c_Resigned" class="ewCell form-group">
	<label for="sv_Resigned" class="ewSearchCaption ewLabel"><?php echo $Page->Resigned->FldCaption() ?></label>
	<span class="ewSearchField">
<?php $Page->Resigned->EditAttrs["onchange"] = "ewrForms(this).Submit(); " . @$Page->Resigned->EditAttrs["onchange"]; ?>
<?php ewr_PrependClass($Page->Resigned->EditAttrs["class"], "form-control"); ?>
<select data-table="IRBA_With_Search" data-field="x_Resigned" data-value-separator="<?php echo ewr_HtmlEncode(is_array($Page->Resigned->DisplayValueSeparator) ? json_encode($Page->Resigned->DisplayValueSeparator) : $Page->Resigned->DisplayValueSeparator) ?>" id="sv_Resigned" name="sv_Resigned"<?php echo $Page->Resigned->EditAttributes() ?>>
<option value=""><?php echo $ReportLanguage->Phrase("PleaseSelect") ?></option>
<?php
	$cntf = is_array($Page->Resigned->AdvancedFilters) ? count($Page->Resigned->AdvancedFilters) : 0;
	$cntd = is_array($Page->Resigned->DropDownList) ? count($Page->Resigned->DropDownList) : 0;
	$totcnt = $cntf + $cntd;
	$wrkcnt = 0;
	if ($cntf > 0) {
		foreach ($Page->Resigned->AdvancedFilters as $filter) {
			if ($filter->Enabled) {
				$selwrk = ewr_MatchedFilterValue($Page->Resigned->DropDownValue, $filter->ID) ? " selected" : "";
?>
...
<?php
	...
	for ($i = 0; $i < $cntd; $i++) {
		$selwrk = " selected";
?>
<option value="<?php echo $Page->Resigned->DropDownList[$i] ?>"<?php echo $selwrk ?>><?php echo ewr_DropDownDisplayValue($Page->Resigned->DropDownList[$i], "", 0) ?></option>
<?php
		$wrkcnt += 1;
	}
?>
</select>
<input type="hidden" name="s_sv_Resigned" id="s_sv_Resigned" value="<?php echo $Page->Resigned->LookupFilterQuery() ?>">
<script type="text/javascript">
fIRBA_With_Searchsummary.Lists["sv_Resigned"].Options = <?php echo ewr_ArrayToJson($Page->Resigned->LookupFilterOptions) ?>;
</script>
</span>
</div>
</div>
...
<script type="text/javascript">
fIRBA_With_Searchsummary.Init();
fIRBA_With_Searchsummary.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php } ?>
<!-- Report grid (begin) -->
<div id="gmp_IRBA_With_Search" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<table class="<?php echo $Page->ReportTableClass ?>" style="width: 3630px;">
<thead>
	<!-- Table header -->
	<tr class="ewTableHeader">
<?php if ($Page->Client_Name->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Client_Name"><div class="IRBA_With_Search_Client_Name"><span class="ewTableHeaderCaption"><?php echo $Page->Client_Name->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Client_Name">
<?php if ($Page->SortUrl($Page->Client_Name) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_Client_Name">
			<span class="ewTableHeaderCaption"><?php echo $Page->Client_Name->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_Client_Name" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Client_Name) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Client_Name->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Group_Structure"><div class="IRBA_With_Search_Group_Structure"><span class="ewTableHeaderCaption"><?php echo $Page->Group_Structure->FldCaption() ?></span></div>
	<hr>Highest level SA parent entity, audited by the SA firm
...
<?php if ($Page->SortUrl($Page->Group_Structure) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_Group_Structure">
			<span class="ewTableHeaderCaption"><?php echo $Page->Group_Structure->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_Group_Structure" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Group_Structure) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Group_Structure->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Client_CIPC"><div class="IRBA_With_Search_Client_CIPC"><span class="ewTableHeaderCaption"><?php echo $Page->Client_CIPC->FldCaption() ?></span></div>
	<hr>
	Registration No
...
<?php if ($Page->SortUrl($Page->Client_CIPC) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_Client_CIPC">
			<span class="ewTableHeaderCaption"><?php echo $Page->Client_CIPC->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_Client_CIPC" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Client_CIPC) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Client_CIPC->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Companies_Act"><div class="IRBA_With_Search_Companies_Act"><span class="ewTableHeaderCaption"><?php echo $Page->Companies_Act->FldCaption() ?></span></div>
	<hr>
...
<?php if ($Page->SortUrl($Page->Companies_Act) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_Companies_Act">
			<span class="ewTableHeaderCaption"><?php echo $Page->Companies_Act->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_Companies_Act" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Companies_Act) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Companies_Act->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="IRBA_Code"><div class="IRBA_With_Search_IRBA_Code"><span class="ewTableHeaderCaption"><?php echo $Page->IRBA_Code->FldCaption() ?></span></div>
	<hr>
...
<?php if ($Page->SortUrl($Page->IRBA_Code) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_IRBA_Code">
			<span class="ewTableHeaderCaption"><?php echo $Page->IRBA_Code->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_IRBA_Code" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->IRBA_Code) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->IRBA_Code->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="ISQC_1"><div class="IRBA_With_Search_ISQC_1"><span class="ewTableHeaderCaption"><?php echo $Page->ISQC_1->FldCaption() ?></span></div>
	<hr>
...
<?php if ($Page->SortUrl($Page->ISQC_1) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_ISQC_1">
			<span class="ewTableHeaderCaption"><?php echo $Page->ISQC_1->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_ISQC_1" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->ISQC_1) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->ISQC_1->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="ISQC_2"><div class="IRBA_With_Search_ISQC_2"><span class="ewTableHeaderCaption"><?php echo $Page->ISQC_2->FldCaption() ?></span></div>
	<hr>
...
<?php if ($Page->SortUrl($Page->ISQC_2) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_ISQC_2">
			<span class="ewTableHeaderCaption"><?php echo $Page->ISQC_2->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_ISQC_2" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->ISQC_2) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->ISQC_2->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Assurance_Work"><div class="IRBA_With_Search_Assurance_Work"><span class="ewTableHeaderCaption"><?php echo $Page->Assurance_Work->FldCaption() ?></span></div>
	<hr>
	Refer to Note 2 C
...
<?php if ($Page->SortUrl($Page->Assurance_Work) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_Assurance_Work">
			<span class="ewTableHeaderCaption"><?php echo $Page->Assurance_Work->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_Assurance_Work" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Assurance_Work) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Assurance_Work->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Amount_Billed"><div class="IRBA_With_Search_Amount_Billed"><span class="ewTableHeaderCaption"><?php echo $Page->Amount_Billed->FldCaption() ?></span></div>
	<hr>
...
<?php if ($Page->SortUrl($Page->Amount_Billed) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_Amount_Billed">
			<span class="ewTableHeaderCaption"><?php echo $Page->Amount_Billed->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_Amount_Billed" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Amount_Billed) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Amount_Billed->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="JSE_Listed"><div class="IRBA_With_Search_JSE_Listed"><span class="ewTableHeaderCaption"><?php echo $Page->JSE_Listed->FldCaption() ?></span></div>
	<hr>
	(L / SL)
...
<?php if ($Page->SortUrl($Page->JSE_Listed) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_JSE_Listed">
			<span class="ewTableHeaderCaption"><?php echo $Page->JSE_Listed->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_JSE_Listed" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->JSE_Listed) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->JSE_Listed->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Industry"><div class="IRBA_With_Search_Industry"><span class="ewTableHeaderCaption"><?php echo $Page->Industry->FldCaption() ?></span></div>
	<hr>
	Refer to Note 2 F
...
<?php if ($Page->SortUrl($Page->Industry) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_Industry">
			<span class="ewTableHeaderCaption"><?php echo $Page->Industry->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_Industry" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Industry) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Industry->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Period_end_of_AFS"><div class="IRBA_With_Search_Period_end_of_AFS"><span class="ewTableHeaderCaption"><?php echo $Page->Period_end_of_AFS->FldCaption() ?></span></div>
	<hr>
	(DD/MM/CCYY)
...
<?php if ($Page->SortUrl($Page->Period_end_of_AFS) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_Period_end_of_AFS">
			<span class="ewTableHeaderCaption"><?php echo $Page->Period_end_of_AFS->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_Period_end_of_AFS" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Period_end_of_AFS) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Period_end_of_AFS->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Date_Opinion_was_signed"><div class="IRBA_With_Search_Date_Opinion_was_signed"><span class="ewTableHeaderCaption"><?php echo $Page->Date_Opinion_was_signed->FldCaption() ?></span></div>
	<hr>
	(DD/MM/CCYY)
...
<?php if ($Page->SortUrl($Page->Date_Opinion_was_signed) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_Date_Opinion_was_signed">
			<span class="ewTableHeaderCaption"><?php echo $Page->Date_Opinion_was_signed->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_Date_Opinion_was_signed" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Date_Opinion_was_signed) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Date_Opinion_was_signed->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Partner"><div class="IRBA_With_Search_Partner"><span class="ewTableHeaderCaption"><?php echo $Page->Partner->FldCaption() ?></span></div>
	<hr>
...
<?php if ($Page->SortUrl($Page->Partner) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_Partner">
			<span class="ewTableHeaderCaption"><?php echo $Page->Partner->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_Partner" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Partner) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Partner->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="IRBA"><div class="IRBA_With_Search_IRBA"><span class="ewTableHeaderCaption"><?php echo $Page->IRBA->FldCaption() ?></span></div>
	<hr>
...
<?php if ($Page->SortUrl($Page->IRBA) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_IRBA">
			<span class="ewTableHeaderCaption"><?php echo $Page->IRBA->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_IRBA" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->IRBA) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->IRBA->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Next_IRBA"><div class="IRBA_With_Search_Next_IRBA"><span class="ewTableHeaderCaption"><?php echo $Page->Next_IRBA->FldCaption() ?></span></div>
	<hr>
...
<?php if ($Page->SortUrl($Page->Next_IRBA) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_Next_IRBA">
			<span class="ewTableHeaderCaption"><?php echo $Page->Next_IRBA->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_Next_IRBA" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Next_IRBA) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Next_IRBA->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Accounting_Framework"><div class="IRBA_With_Search_Accounting_Framework"><span class="ewTableHeaderCaption"><?php echo $Page->Accounting_Framework->FldCaption() ?></span></div>
	<hr>
...
<?php if ($Page->SortUrl($Page->Accounting_Framework) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_Accounting_Framework">
			<span class="ewTableHeaderCaption"><?php echo $Page->Accounting_Framework->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_Accounting_Framework" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Accounting_Framework) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Accounting_Framework->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Client_Turnover"><div class="IRBA_With_Search_Client_Turnover"><span class="ewTableHeaderCaption"><?php echo $Page->Client_Turnover->FldCaption() ?></span></div>
	<hr>
...
<?php if ($Page->SortUrl($Page->Client_Turnover) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_Client_Turnover">
			<span class="ewTableHeaderCaption"><?php echo $Page->Client_Turnover->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_Client_Turnover" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Client_Turnover) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Client_Turnover->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->Resigned->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Resigned"><div class="IRBA_With_Search_Resigned"><span class="ewTableHeaderCaption"><?php echo $Page->Resigned->FldCaption() ?></span></div>
	<hr>
...
<?php } else { ?>
	<td data-field="Resigned">
<?php if ($Page->SortUrl($Page->Resigned) == "") { ?>
		<div class="ewTableHeaderBtn IRBA_With_Search_Resigned">
			<span class="ewTableHeaderCaption"><?php echo $Page->Resigned->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer IRBA_With_Search_Resigned" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Resigned) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Resigned->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->Resigned->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->Resigned->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
<hr>
...
<?php } ?>
<?php if ($Page->Resigned->Visible) { ?>
		<td data-field="Resigned"<?php echo $Page->Resigned->CellAttributes() ?>>
<span<?php echo $Page->Resigned->ViewAttributes() ?>><?php echo $Page->Resigned->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
...
<?php include "IRBA_With_Searchsmrypager.php" ?>
