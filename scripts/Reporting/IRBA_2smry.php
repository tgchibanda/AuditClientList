...
//		$item->Visible = TRUE;
	...
	function Page_Main() {
		...
		$this->Partner->SelectionList = "";
		$this->Partner->DefaultSelectionList = "";
		$this->Partner->ValueList = "";
	...
	function GetRow($opt) {
		...
		if (!$rs->EOF) {
			...
			$this->Val[3] = $this->Companies_Act->CurrentValue;
			$this->Val[4] = $this->IRBA_Code->CurrentValue;
			$this->Val[5] = $this->ISQC_1->CurrentValue;
			$this->Val[6] = $this->ISQC_2->CurrentValue;
			$this->Val[7] = $this->Assurance_Work->CurrentValue;
			$this->Val[8] = $this->Amount_Billed->CurrentValue;
			$this->Val[9] = $this->JSE_Listed->CurrentValue;
			$this->Val[10] = $this->Industry->CurrentValue;
			$this->Val[11] = $this->Period_end_of_AFS->CurrentValue;
			$this->Val[12] = $this->Date_Opinion_was_signed->CurrentValue;
			$this->Val[13] = $this->Partner->CurrentValue;
			$this->Val[14] = $this->Next_IRBA->CurrentValue;
			$this->Val[15] = $this->Accounting_Framework->CurrentValue;
			$this->Val[16] = $this->Client_Turnover->CurrentValue;
		} else {
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			// Check popup name
			// Build distinct values for Partner

			if ($popupname == 'IRBA_2_Partner') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;

				// Call Page Filtering event
				$this->Page_Filtering($this->Partner, $sFilter, "popup");
				$sSql = ewr_BuildReportSql($this->Partner->SqlSelect, $this->getSqlWhere(), $this->getSqlGroupBy(), $this->getSqlHaving(), $this->Partner->SqlOrderBy, $sFilter, "");
				$rswrk = $conn->Execute($sSql);
				while ($rswrk && !$rswrk->EOF) {
					$this->Partner->setDbValue($rswrk->fields[0]);
					$this->Partner->ViewValue = @$rswrk->fields[1];
					if (is_null($this->Partner->CurrentValue)) {
						$bNullValue = TRUE;
					} elseif ($this->Partner->CurrentValue == "") {
						$bEmptyValue = TRUE;
					} else {
						ewr_SetupDistinctValues($this->Partner->ValueList, $this->Partner->CurrentValue, $this->Partner->ViewValue, FALSE, $this->Partner->FldDelimiter);
					}
					$rswrk->MoveNext();
				}
				if ($rswrk)
					$rswrk->Close();
				if ($bEmptyValue)
					ewr_SetupDistinctValues($this->Partner->ValueList, EWR_EMPTY_VALUE, $ReportLanguage->Phrase("EmptyLabel"), FALSE);
				if ($bNullValue)
					ewr_SetupDistinctValues($this->Partner->ValueList, EWR_NULL_VALUE, $ReportLanguage->Phrase("NullLabel"), FALSE);
				$fld = &$this->Partner;
			}
	...
	function SetupPopup() {
		...
		// Get 'reset' command
			...
			if (strtolower($sCmd) == "reset") {
				$this->ClearSessionSelection('Partner');
				$this->ResetPager();
			}
		}

		// Load selection criteria to array
		// Get Partner selected values

		if (is_array(@$_SESSION["sel_IRBA_2_Partner"])) {
			$this->LoadSelectionFromSession('Partner');
		} elseif (@$_SESSION["sel_IRBA_2_Partner"] == EWR_INIT_VALUE) { // Select all
			$this->Partner->SelectionList = "";
		}
	}
	...
	function GetExtendedFilter() {
		...
		if ($bPostBack) {
			...
			// Set/clear dropdown for field Partner
			if ($this->PopupName == 'IRBA_2_Partner' && $this->PopupValue <> "") {
				if ($this->PopupValue == EWR_INIT_VALUE)
					$this->Partner->DropDownValue = EWR_ALL_VALUE;
				else
					$this->Partner->DropDownValue = $this->PopupValue;
				$bRestoreSession = FALSE; // Do not restore
			} elseif ($this->ClearExtFilter == 'IRBA_2_Partner') {
				$this->SetSessionDropDownValue(EWR_INIT_VALUE, '', 'Partner');
			}
		...
		// Reset search command
			...
			// Load default values
			$this->SetSessionDropDownValue($this->Partner->DropDownValue, $this->Partner->SearchOperator, 'Partner'); // Field Partner
			...
			// Field Partner
			if ($this->GetDropDownValue($this->Partner)) {
				$bSetupFilter = TRUE;
			} elseif ($this->Partner->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_IRBA_2_Partner'])) {
				$bSetupFilter = TRUE;
			}
		...
		// Restore session
		if ($bRestoreSession) {
			$this->GetSessionDropDownValue($this->Partner); // Field Partner
		}
		...
		// Build SQL
		$this->BuildDropDownFilter($this->Partner, $sFilter, $this->Partner->SearchOperator, FALSE, TRUE); // Field Partner
		...
		if ($bSetupFilter) {
			...
			// Field Partner
			$sWrk = "";
			$this->BuildDropDownFilter($this->Partner, $sWrk, $this->Partner->SearchOperator);
			ewr_LoadSelectionFromFilter($this->Partner, $sWrk, $this->Partner->SelectionList, $this->Partner->DropDownValue);
			$_SESSION['sel_IRBA_2_Partner'] = ($this->Partner->SelectionList == "") ? EWR_INIT_VALUE : $this->Partner->SelectionList;
		}

		// Field Partner
		ewr_LoadDropDownList($this->Partner->DropDownList, $this->Partner->DropDownValue);
		return $sFilter;
	}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_IRBA_2_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_IRBA_2_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_IRBA_2_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_IRBA_2_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_IRBA_2_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_IRBA_2_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_IRBA_2_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_IRBA_2_' . $parm] = $sv;
		$_SESSION['so_IRBA_2_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_IRBA_2_' . $parm] = $sv1;
		$_SESSION['so_IRBA_2_' . $parm] = $so1;
		$_SESSION['sc_IRBA_2_' . $parm] = $sc;
		$_SESSION['sv2_IRBA_2_' . $parm] = $sv2;
		$_SESSION['so2_IRBA_2_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_IRBA_2_$parm"] = "";
		$_SESSION["rf_IRBA_2_$parm"] = "";
		$_SESSION["rt_IRBA_2_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_IRBA_2_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_IRBA_2_$parm"];
		$fld->RangeTo = @$_SESSION["rt_IRBA_2_$parm"];
	}
	...
	function LoadDefaultFilters() {
		...
		*/
		...
		// Field Partner
		$this->Partner->DefaultDropDownValue = EWR_INIT_VALUE;
		if (!$this->SearchCommand) $this->Partner->DropDownValue = $this->Partner->DefaultDropDownValue;
		$sWrk = "";
		$this->BuildDropDownFilter($this->Partner, $sWrk, $this->Partner->SearchOperator, TRUE);
		ewr_LoadSelectionFromFilter($this->Partner, $sWrk, $this->Partner->DefaultSelectionList);
		if (!$this->SearchCommand) $this->Partner->SelectionList = $this->Partner->DefaultSelectionList;
		/**
		...
		*/
		...
		// Field Partner
		// $this->Partner->DefaultSelectionList = array("val1", "val2");
	...
	function CheckFilter() {
		...
		// Check Partner extended filter
		if ($this->NonTextFilterApplied($this->Partner))
			return TRUE;

		// Check Partner popup filter
		if (!ewr_MatchedArray($this->Partner->DefaultSelectionList, $this->Partner->SelectionList))
			return TRUE;
		return FALSE;
	}
	...
	function ShowFilterList($showDate = FALSE) {
		...
		$sExtWrk = "";
		$sWrk = "";
		$this->BuildDropDownFilter($this->Partner, $sExtWrk, $this->Partner->SearchOperator);
		if (is_array($this->Partner->SelectionList))
			$sWrk = ewr_JoinArray($this->Partner->SelectionList, ", ", EWR_DATATYPE_STRING, 0, $this->DBID);
		$sFilter = "";
		if ($sExtWrk <> "")
		...
		if ($sFilter <> "")
			$sFilterList .= "<div><span class=\"ewFilterCaption\">" . $this->Partner->FldCaption() . "</span>" . $sFilter . "</div>";
		$divstyle = "";
		$divdataclass = "";
	...
	function GetFilterList() {
		...
		// Field Partner
		$sWrk = "";
		$sWrk = ($this->Partner->DropDownValue <> EWR_INIT_VALUE) ? $this->Partner->DropDownValue : "";
		if (is_array($sWrk))
			$sWrk = implode("||", $sWrk);
		if ($sWrk <> "")
			$sWrk = "\"sv_Partner\":\"" . ewr_JsEncode2($sWrk) . "\"";
		if ($sWrk == "") {
			$sWrk = ($this->Partner->SelectionList <> EWR_INIT_VALUE) ? $this->Partner->SelectionList : "";
			if (is_array($sWrk))
				$sWrk = implode("||", $sWrk);
			if ($sWrk <> "")
				$sWrk = "\"sel_Partner\":\"" . ewr_JsEncode2($sWrk) . "\"";
		}
		if ($sWrk <> "") {
	...
	function SetupFilterList($filter) {
		...
		if (array_key_exists("sv_Partner", $filter)) {
			$sWrk = $filter["sv_Partner"];
			if (strpos($sWrk, "||") !== FALSE)
				$sWrk = explode("||", $sWrk);
			$this->SetSessionDropDownValue($sWrk, @$filter["so_Partner"], "Partner");
			$bRestoreFilter = TRUE;
		}
		if (array_key_exists("sel_Partner", $filter)) {
			$sWrk = $filter["sel_Partner"];
			$sWrk = explode("||", $sWrk);
			$this->Partner->SelectionList = $sWrk;
			$_SESSION["sel_IRBA_2_Partner"] = $sWrk;
			$this->SetSessionDropDownValue(EWR_INIT_VALUE, "", "Partner"); // Clear drop down
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionDropDownValue(EWR_INIT_VALUE, "", "Partner");
			$this->Partner->SelectionList = "";
			$_SESSION["sel_IRBA_2_Partner"] = "";
		}
		return TRUE;
	}
	...
	function GetPopupFilter() {
		...
		if ($this->DrillDown)
			return "";
		if (!$this->DropDownFilterExist($this->Partner, $this->Partner->SearchOperator)) {
			if (is_array($this->Partner->SelectionList)) {
				$sFilter = ewr_FilterSql($this->Partner, "`Partner`", EWR_DATATYPE_STRING, $this->DBID);

				// Call Page Filtering event
				$this->Page_Filtering($this->Partner, $sFilter, "popup");
				$this->Partner->CurrentFilter = $sFilter;
...
// Form object
var CurrentForm = fIRBA_2summary = new ewr_Form("fIRBA_2summary");

// Validate method
fIRBA_2summary.Validate = function() {
...
fIRBA_2summary.Form_CustomValidate = 
...
fIRBA_2summary.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fIRBA_2summary.ValidateRequired = false; // No JavaScript validation
<?php } ?>

// Use Ajax
fIRBA_2summary.Lists["sv_Partner"] = {"LinkField":"sv_Partner","Ajax":true,"DisplayFields":["sv_Partner","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown && !$Page->ShowReportInDashboard) { ?>
<script type="text/javascript">
...
<?php if ($Page->Export == "" && !$Page->DrillDown && !$Page->ShowReportInDashboard) { ?>
<!-- Search form (begin) -->
<form name="fIRBA_2summary" id="fIRBA_2summary" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fIRBA_2summary_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<div id="r_1" class="ewRow">
<div id="c_Partner" class="ewCell form-group">
	<label for="sv_Partner" class="ewSearchCaption ewLabel"><?php echo $Page->Partner->FldCaption() ?></label>
	<span class="ewSearchField">
<?php $Page->Partner->EditAttrs["onchange"] = "ewrForms(this).Submit(); " . @$Page->Partner->EditAttrs["onchange"]; ?>
<?php ewr_PrependClass($Page->Partner->EditAttrs["class"], "form-control"); ?>
<select data-table="IRBA_2" data-field="x_Partner" data-value-separator="<?php echo ewr_HtmlEncode(is_array($Page->Partner->DisplayValueSeparator) ? json_encode($Page->Partner->DisplayValueSeparator) : $Page->Partner->DisplayValueSeparator) ?>" id="sv_Partner" name="sv_Partner"<?php echo $Page->Partner->EditAttributes() ?>>
<option value=""><?php echo $ReportLanguage->Phrase("PleaseSelect") ?></option>
<?php
	$cntf = is_array($Page->Partner->AdvancedFilters) ? count($Page->Partner->AdvancedFilters) : 0;
	$cntd = is_array($Page->Partner->DropDownList) ? count($Page->Partner->DropDownList) : 0;
	$totcnt = $cntf + $cntd;
	$wrkcnt = 0;
	if ($cntf > 0) {
		foreach ($Page->Partner->AdvancedFilters as $filter) {
			if ($filter->Enabled) {
				$selwrk = ewr_MatchedFilterValue($Page->Partner->DropDownValue, $filter->ID) ? " selected" : "";
?>
...
<?php
	...
	for ($i = 0; $i < $cntd; $i++) {
		$selwrk = " selected";
?>
<option value="<?php echo $Page->Partner->DropDownList[$i] ?>"<?php echo $selwrk ?>><?php echo ewr_DropDownDisplayValue($Page->Partner->DropDownList[$i], "", 0) ?></option>
<?php
		$wrkcnt += 1;
	}
?>
</select>
<input type="hidden" name="s_sv_Partner" id="s_sv_Partner" value="<?php echo $Page->Partner->LookupFilterQuery() ?>">
<script type="text/javascript">
fIRBA_2summary.Lists["sv_Partner"].Options = <?php echo ewr_ArrayToJson($Page->Partner->LookupFilterOptions) ?>;
</script>
</span>
</div>
</div>
...
<script type="text/javascript">
fIRBA_2summary.Init();
fIRBA_2summary.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php } ?>
<table class="<?php echo $Page->ReportTableClass ?>" style="width: 3330px;">
<thead>
	<!-- Table header -->
	<tr class="ewTableHeader">
<?php if ($Page->Client_Name->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Client_Name" style="width: 300px;"><div class="IRBA_2_Client_Name"><span class="ewTableHeaderCaption"><?php echo $Page->Client_Name->FldCaption() ?></span></div>
	<hr></td>
<?php } else { ?>
	<td data-field="Client_Name" style="width: 300px;">
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	...
	<hr>Public Interest Score (PIS)</td>
<?php } else { ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	...
	<hr>Public Interest Entity (PIE) Yes/No</td>
<?php } else { ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	...
	<hr>Was an Engagement Quality Control Review (EQCR) performed?</td>
<?php } else { ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	...
	<hr>Was a Monitoring review performed?</td>
<?php } else { ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Amount_Billed"><div class="IRBA_2_Amount_Billed"><span class="ewTableHeaderCaption"><?php echo $Page->Amount_Billed->FldCaption() ?></span></div></td>
<?php } else { ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Partner"><div class="IRBA_2_Partner"><span class="ewTableHeaderCaption"><?php echo $Page->Partner->FldCaption() ?></span></div></td>
<?php } else { ?>
...
<?php if ($Page->SortUrl($Page->Partner) == "") { ?>
	...
	<?php if (!$Page->ShowReportInDashboard) { ?>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, { name: 'IRBA_2_Partner', range: false, from: '<?php echo $Page->Partner->RangeFrom; ?>', to: '<?php echo $Page->Partner->RangeTo; ?>', url: 'IRBA_2smry.php' });" id="x_Partner<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
<?php } else { ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	...
	<hr>Cycle- public interest 3 yrs/non public interest 6 years</td>
<?php } else { ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Accounting_Framework"><div class="IRBA_2_Accounting_Framework"><span class="ewTableHeaderCaption"><?php echo $Page->Accounting_Framework->FldCaption() ?></span></div></td>
<?php } else { ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Client_Turnover"><div class="IRBA_2_Client_Turnover"><span class="ewTableHeaderCaption"><?php echo $Page->Client_Turnover->FldCaption() ?></span></div></td>
<?php } else { ?>
